...
func main() {
	...
	cron.Cron(cfg.Crontab.LowPerformanceAlarmTime).StartImmediately().SingletonMode().Do(lowPerformanceAlarm)
	cron.Cron(cfg.Crontab.SumPerformanceAlarmTime).StartImmediately().SingletonMode().Do(sumPerformanceAlarm)
	cron.StartBlocking()
}
